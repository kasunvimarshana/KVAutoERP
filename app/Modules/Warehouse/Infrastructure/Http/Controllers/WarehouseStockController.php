...
use Modules\Warehouse\Infrastructure\Http\Resources\StockMovementResource;
use Symfony\Component\HttpFoundation\Response as HttpResponse;

class WarehouseStockController extends AuthorizedController
{
    public function __construct(
        private readonly RecordWarehouseStockMovementServiceInterface $recordWarehouseStockMovementService,
        private readonly FindWarehouseStockMovementServiceInterface $findWarehouseStockMovementService,
        private readonly FindWarehouseStockLevelServiceInterface $findWarehouseStockLevelService,
    ) {}
    ...
    public function movements(ListWarehouseStockMovementRequest $request, int $warehouse): JsonResponse
    {
        ...
        $movements = $this->findWarehouseStockMovementService->listByWarehouse(
    ...
    public function storeMovement(StoreWarehouseStockMovementRequest $request, int $warehouse): JsonResponse
    {
        ...
        $movement = $this->recordWarehouseStockMovementService->execute($payload);
    ...
    public function stockLevels(ListWarehouseStockLevelRequest $request, int $warehouse): JsonResponse
    {
        ...
        $levels = $this->findWarehouseStockLevelService->listByWarehouse(
            tenantId: (int) $validated['tenant_id'],
            warehouseId: $warehouse,
            perPage: (int) ($validated['per_page'] ?? 15),
            page: (int) ($validated['page'] ?? 1),
