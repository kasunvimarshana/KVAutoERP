...
class EloquentReturnRequestRepository implements ReturnRequestRepositoryInterface {
    public function __construct(private readonly ReturnRequestModel $model, private readonly ReturnLineModel $lineModel) {}
    private function toEntity(ReturnRequestModel $m): ReturnRequest {
        return new ReturnRequest($m->id,$m->tenant_id,$m->return_type,$m->reference_id,$m->return_number,$m->status,
            $m->reason,$m->notes,$m->processed_by,$m->lines->toArray(),$m->processed_at,$m->created_at,$m->updated_at);
    }
    public function findById(int $id): ?ReturnRequest { $m=$this->model->newQuery()->with('lines')->find($id); return $m?$this->toEntity($m):null; }
    ...
    public function create(array $data, array $lines): ReturnRequest {
        return DB::transaction(function() use ($data,$lines) {
            ...
            foreach($lines as $l) $this->lineModel->newQuery()->create(array_merge($l,['return_request_id'=>$m->id]));
            return $this->findById($m->id);
        });
    }
    public function update(int $id, array $data): ?ReturnRequest { $m=$this->model->newQuery()->find($id); if(!$m)return null; $m->update($data); return $this->findById($id); }
