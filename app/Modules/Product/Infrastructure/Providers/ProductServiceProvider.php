...
class ProductServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        $this->app->bind(
            ...
            fn ($app) => new EloquentCategoryRepository($app->make(CategoryModel::class))
        );

        $this->app->bind(
            ProductRepositoryInterface::class,
            fn ($app) => new EloquentProductRepository($app->make(ProductModel::class))
        );

        $this->app->bind(
            ProductVariantRepositoryInterface::class,
            fn ($app) => new EloquentProductVariantRepository($app->make(ProductVariantModel::class))
        );

        $this->app->bind(
            ProductComponentRepositoryInterface::class,
            fn ($app) => new EloquentProductComponentRepository($app->make(ProductComponentModel::class))
        );

        $this->app->bind(
            CategoryServiceInterface::class,
            fn ($app) => new CategoryService($app->make(CategoryRepositoryInterface::class))
        );

        $this->app->bind(
            ProductServiceInterface::class,
            fn ($app) => new ProductService($app->make(ProductRepositoryInterface::class))
        );

        $this->app->bind(
            ProductVariantServiceInterface::class,
            fn ($app) => new ProductVariantService($app->make(ProductVariantRepositoryInterface::class))
        );

        $this->app->bind(
            ProductComponentServiceInterface::class,
            fn ($app) => new ProductComponentService($app->make(ProductComponentRepositoryInterface::class))
        );
    }
