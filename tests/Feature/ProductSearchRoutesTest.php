...
class ProductSearchRoutesTest extends TestCase
{
    ...
    public function test_product_search_endpoints_require_authentication(): void
    {
        $this->getJson('/api/products/search')->assertStatus(HttpResponse::HTTP_UNAUTHORIZED);
        $this->postJson('/api/products/search/rebuild')->assertStatus(HttpResponse::HTTP_UNAUTHORIZED);
    }

    public function test_product_search_routes_keep_expected_middleware_contract(): void
    {
        $routes = app('router')->getRoutes();

        $this->assertRouteUsesMiddleware($this->findRoute($routes, 'api/products/search', 'GET'), ['auth:api', 'resolve.tenant']);
        $this->assertRouteUsesMiddleware($this->findRoute($routes, 'api/products/search/rebuild', 'POST'), ['auth:api', 'resolve.tenant']);
    }
