...
class EmployeeRoutesTest extends TestCase
{
    ...
    {
        $this->getJson('/api/employees')->assertStatus(HttpResponse::HTTP_UNAUTHORIZED);
        $this->getJson('/api/employees/1')->assertStatus(HttpResponse::HTTP_UNAUTHORIZED);
    }

    public function test_employee_routes_keep_expected_middleware_contract(): void
    {
        $routes = app('router')->getRoutes();

        $this->assertRouteUsesMiddleware($this->findRoute($routes, 'api/employees', 'GET'), ['auth.configured', 'resolve.tenant']);
        $this->assertRouteUsesMiddleware($this->findRoute($routes, 'api/employees/{employee}', 'GET'), ['auth.configured', 'resolve.tenant']);
    }
