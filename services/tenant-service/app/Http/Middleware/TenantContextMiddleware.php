...
class TenantContextMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($tenant === null) {
            return response()->json([
                'success' => false,
                'message' => 'Tenant could not be resolved. Provide a valid X-Tenant-ID header.',
            ], 400);
        }
    ...
    private function resolveTenant(Request $request): ?Tenant
    {
        ...
        if ($tenantId = $request->header('X-Tenant-ID')) {
            return $this->fromCache('id', (string) $tenantId, fn () => $this->tenantRepository->findById($tenantId));
        }

        // 2. Slug header
        if ($slug = $request->header('X-Tenant-Slug')) {
            return $this->fromCache('slug', (string) $slug, fn () => $this->tenantRepository->findBySlug($slug));
        }
