...
final class VerifyJwtMiddleware
{
    ...
    {
        $publicKeyPath = config('product_service.jwt.public_key');
        ...
        $leewaySeconds = (int) config('product_service.jwt.leeway_seconds', 30);
    ...
    {
        try {
            $prefix = config('product_service.revocation.jti_prefix', 'jti:');
    ...
    {
        try {
            $prefix          = config('product_service.revocation.version_prefix', 'token_version:');
