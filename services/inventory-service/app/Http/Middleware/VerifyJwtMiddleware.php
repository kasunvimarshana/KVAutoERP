 ...
 *
 * The product service never holds the private key — it only verifies
 ...
 *   5. Attach verified claims to `$request->attributes` as `jwt_claims`.
 *
...
final class VerifyJwtMiddleware
{
    ...
    {
        $publicKeyPath = config('inventory_service.jwt.public_key');
        ...
        $leewaySeconds = (int) config('inventory_service.jwt.leeway_seconds', 30);
    ...
    {
        try {
            $prefix = config('inventory_service.revocation.jti_prefix', 'jti:');
    ...
    {
        try {
            $prefix          = config('inventory_service.revocation.version_prefix', 'token_version:');
